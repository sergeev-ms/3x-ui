...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls "https://api.github.com/repos/sergeev_ms/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${red}Failed to fetch x-ui version, it may be due to GitHub API restrictions, please try it later${plain}"
            exit 1
        fi
        echo -e "Got x-ui latest version: ${tag_version}, beginning the installation..."
        wget -N -O /usr/local/x-ui-linux-$(arch).tar.gz https://github.com/sergeev_ms/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        url="https://github.com/sergeev_ms/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
    ...
    wget -O /usr/bin/x-ui https://raw.githubusercontent.com/sergeev_ms/3x-ui/main/x-ui.sh
